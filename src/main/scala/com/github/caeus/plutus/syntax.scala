...
object syntax {
  object P {
    ...
    def apply[In, Repr](seq: SeqLike[In, Repr]): Packer[In, Unit] =
      fromList(seq.toList)
  ...
  implicit class PackerOps[In, Out](val value: Packer[In, Out]) extends AnyVal {
    ...
    def ~[Out1](next: => Packer[In, Out1]): Packer[In, Unit] =
      value.flatMap(_ => next.map(_ => ()))

    def ~>[Out1](next: => Packer[In, Out1]): Packer[In, Out1] =
      value.flatMap(_ => next)

    def <~>[Out1](next: => Packer[In, Out1]): Packer[In, (Out, Out1)] =
      value.flatMap(a => next.map(b => a -> b))

    def <~[Out1](next: => Packer[In, Out1]): Packer[In, Out] =
      value.flatMap(a => next.map(_ => a))

    def ? : Packer[In, Option[Out]] =
    ...
    def log(name: String): Packer[In, Out] = new Packer[In, Out] {
      override def take(input: List[In]): PackerResult[In, Out] = {
        println(
          s"packer $name will take ${input.take(5).mkString("([", "][", "]...")} ${input.size}"
        )
        val result = value.take(input)
        result match {
          case Done(value, taken) =>
            println(
              s"packer $name returned with value $value and took $taken tokens leaving a remaining ${input
                .slice(taken, taken + 5)
                .mkString("([", "][", "]...")}"
            )
          case Failed(m, taken) =>
            println(
              s"packer $name failed for input ${input.take(5).mkString("([", "][", "]...")}... $taken ahead with message: $m"
            )
        }
        result
      }
    ...
    def rep(min: Int = 0,
            max: Option[Int] = None,
            sep: Packer[In, _] = pure(())): Packer[In, List[Out]] = {
      if (min > 0)
        for {
