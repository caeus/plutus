...
object PackerResult {
  implicit def ioPlutusOrderingInstance[In, Out]: Ordering[PackerResult[In, Out]] =
    new Ordering[PackerResult[In, Out]] {
...
object Packer {
  ...
  def capture[In, Out](value: Packer[In, Out]): Packer[In, List[In]] = { input: List[In] =>
  ...
  def repeatUpTo[In, Out](
    ...
    def recursive(upTo: Option[Int], accum: List[Out]): Packer[In, List[Out]] = {
      upTo.map(_.compareTo(0)).getOrElse(1) match {
        ...
        case 1 =>
          val sepPacker =
            longestOf[In, Option[Out]](pure(None),
                                       (if (accum.nonEmpty)
